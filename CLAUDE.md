...
`memberSpec.unpackedSz` and `packedSz` are `*int64` rather than `int64`, and the reason is a rule rather than a preference: **a builder must not read a declared zero as an absent value.** A member declaring `UnpackedSize` 0 while carrying a payload is exactly the packed-remainder attack — its leftover bytes are what gets parsed as the next block header — and zero-means-derive silently rewrote that declaration into the member's own length, removing the attack from the vocabulary of the one builder that had always been able to state it. No fixture passed zero at the time, so nothing failed and nothing would have. `new(int64(n))` states a size, `nil` asks for the default, and `TestBuilderPreservesADeclaredZeroSize` pins both.
