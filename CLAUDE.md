...
- **A cut the traversal cannot vouch for must not end the archive cleanly.** `io.LimitedReader` reports `io.EOF` whether it delivered its whole count or the source ran out under it, so neither the scan nor the splice could tell a volume boundary from a truncation. Three mechanisms now separate them: `volume.next()` fails with a wrapped `io.ErrUnexpectedEOF` when the skip completes with `body.N > 0`; `multiVolumePayloadReader.Read` refuses to advance when `volume.bodyShort()` says the cut is inside THIS member's payload, which previously stitched a short volume onto the next one and reported the member complete — and that refusal is reached through `errors.Is(err, io.EOF)`, not an identity comparison, because `io.LimitedReader` returns a bare `io.EOF` only when it has delivered its whole count and otherwise passes the underlying reader's error through verbatim, so a consumer that wraps its end-of-volume EOF (`"volume 3: %w"`, which `io.Reader` permits) skipped the check entirely and got `ErrTruncatedFile` where the archive had told the traversal how many bytes to expect; and `Reader.damaged` records a cut in any other block so that running out of volumes afterwards reports it in `io.EOF`'s place. The scan deliberately continues past a cut — the members beyond it are still readable, and a set arriving with a part missing is ordinary rather than exceptional — so the record of the damage is the only thing standing between that and a caller taking a set with a hole in it for a complete one.
...
- **The 32-bit job runs the tests, and one 32-bit property has exactly zero margin.** `decodeOffset` accumulates an LZ distance in an `int`: an offset slot reaches 63, so the shift term alone reaches `3<<30`, and where `int` is 32 bits that addition wraps before `CopyBytes` ever sees it. It is refused either way, but by different guards — a value that fits is far past `historyLen()`, a value that wrapped is `<= 0` — which makes `CopyBytes`'s `distance <= 0` the 32-bit backstop as well as the `srcIdx` bound its comment describes. What makes the wrap safe is narrow: RAR5's dictionary maxes at 4 GB (the size field is a 4-bit exponent, `128KB<<15`), so the largest distance the format can express is **exactly 2^32** — the same number an `int32` wraps at. Everything in `[2^31, 2^32)` truncates negative and `2^32` truncates to 0, so all of it fails `distance <= 0`; one byte further and a 32-bit build computes a small positive distance from a stream a 64-bit build refuses. `TestOffsetEncodingTopsOutExactlyAtTheInt32Wrap` pins the equality, because it is equality and not proximity: widening `lowoffsetSize5` by a single symbol breaks it. The tests are written against explicit `int64`/`int32` models rather than the host's `int`, for the reason the E8 sign tests give below, and CI now runs the whole suite under `GOARCH=386` (those binaries execute on an amd64 runner; `-race` does not support linux/386, and `GOARCH=arm` stays vet-only because it needs qemu) — before that step the job compiled 32-bit code without executing a single assertion on it.
