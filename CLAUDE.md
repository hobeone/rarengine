...
- **Never let a block header be parsed out of a previous file's payload.** Resynchronising header parsing onto unread payload lets a crafted archive surface a fabricated file entry from `Next()` — name, size and metadata all attacker-chosen, and the header CRC32 is no obstacle because the attacker computes it. This is not only a truncation concern: a block may declare more packed bytes than its file's `UnpackedSize`, so a file that completes *successfully* can still leave a well-formed header sitting in its own payload. `fileReader` owns that remainder (`packed`) and `endFile` drains it on every terminal path, not just failing ones. Any path that refuses a file after reading its block header must likewise drop that file's payload (as `processHeader` does for `ErrUnpSizeUnknown`, using the CRC-checked `h.DataSize`).
- **The packed remainder is tracked per volume, never captured once.** Both engines repoint the single `limitPr` in `processVolumePayloadHeader` instead of returning a fresh `io.LimitReader`, so the count always describes the volume actually being read. A count captured at file start goes stale the moment a file crosses a volume boundary, and discarding a stale count consumes a *later* volume's legitimate header bytes — reintroducing the fabrication above by a different route. `fileReader.packed` must stay the same limiter the engine repoints; giving it a private copy silently restores the staleness.
