...
- **A declared size of zero is not evidence that a block declares nothing.** RAR3's `h.DataSize` carries only the low 32 bits of a packed size, so a block whose true size is an exact multiple of 4 GiB — or one simply built with `ADD_SIZE` zero and the high half elsewhere — reads as declaring no payload at all. Gating the unmeasurable-block refusal on `h.DataSize > 0` therefore let exactly those blocks past it into `discardPayload(0)`, which consumes nothing and leaves the whole payload for the next header read. The flag claiming a large size is what matters; the declared size is not evidence against it. The same reasoning applies to any future guard keyed on a size this library did not itself compute.
- **A refusal that cannot discard must end traversal, not merely report.** Every refusal in these engines drops the offending block's payload and so leaves the stream standing on the next header, which is what makes it safe for the caller to keep calling `Next()`. `ErrRAR3UnmeasurablePayload` is the one refusal that cannot: the byte count it would need is exactly what it failed to read, so the stream stays parked at the start of attacker-chosen bytes. Reporting it without recording `rar3Engine.fatal` left the retry that `Next()`'s own doc comment invites reading its next header out of that payload — the same fabrication the discard rule exists to prevent, reintroduced by the code that added the rule. A new refusal path must therefore answer both questions: what drops the payload, and if nothing can, what stops traversal.
