...
| 24 | **Final measured cost, `main` → finished branch, Ryzen 9 9950X3D, `-count=10`, fixed selection:** `Decompress_Store` **+3.19%**, `Compress` **+3.52%**, `ReaderReset` **+2.77%**, `Solid` ~ (`p=0.123`); geomean **+2.46%** ns/op, throughput **−2.29%**. `+32 B/op` on every benchmark, **allocation count unchanged** (`p=1.000`, all samples equal). | Task 6 Step 7 |
| 25 | **A per-task benchmark cannot see the branch.** Task 4's Step 0 baseline is captured *after* Task 3, so it isolates `volume`'s 32-byte growth — which on this hardware costs ~0 ns/op — and reported "no significant change on three of four benchmarks". The ~3% in row 24 is `Entry.Read`'s `chanClosed`, introduced in Task 3 and invisible to Task 4's brackets. Round 5 flagged this as a missing assumption ("the earlier draft's mistake has been inverted rather than removed"); Task 6 Step 7 exists because of it, and is what produced row 24. Round 5's own bisection had attributed roughly half the cost to each half and was right about the `selectnbrecv` half. | Round 5's missing-assumption note; confirmed at Task 6 Step 7 |
